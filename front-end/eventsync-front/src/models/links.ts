export type links = {
    name: string;
    path: string;
};
export const AlunoLinks = [
    { name: "Home", path: "/dashboard/aluno" },
    { name: "Eventos", path: "/events/aluno" },
    { name: "Atividades", path: "/activities/aluno" },
];
export const professorLinks = [
    { name: "Eventos", path: "/dashboard/professor" },
    // { name: "Eventos", path: "/events/professor" },
];
export const CoordenadorLinks = [
    { name: "Home", path: "/dashboard/coordenador" },
    { name: "Eventos", path: "/events/coordenador" },
    { name: "Atividades", path: "/activities/coordenador" },
];
